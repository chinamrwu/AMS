...
scores <- apply(X1,2,function(v){
      v0 <- v[1:L]
		...
		c(mean(v0),mean(v1),sd(v1)*100/mean(v1))
})
scores <- t(scores)
colnames(scores) <- c("betaN","betaC",'CV')
scores <- data.frame(scores)
scores <- scores[scores$betaN <= 0.2 & scores$betaC >=0.5,]
scores <- scores[order(scores$betaC-scores$betaN,decreasing = T),]
scores$dltBata <- scores$betaC - scores$betaN
...
probScores <- apply(df0,2,function(v){
      v0 <- v[1:L]
		v1 <- v[(L+1):dim(X1)[1]]
      c(mean(v0),mean(v1),mean(v1)-mean(v0))
})
probScores <- t(probScores)
colnames(probScores) <- c('betaN','betaC','dltBeta')
probScores <- data.frame(probScores)
probScores$probId <- rownames(probScores)
probScores <- probScores[,c('probId','betaN','betaC','dltBeta')]
probScores <- sqldf('SELECT A.*,B.Chromosome,B.Start,B.End,B.CGI_Coordinate CGILocation FROM probScores A,probInf B WHERE A.probId=B.probId')

scores$CGI <- rownames(scores)
scores$importance <- 1:dim(scores)[1]

tmp <- sqldf('SELECT A.*,B.Gene_Symbol,B.importance FROM probScores A,scores B WHERE A.CGILocation=B.CGI order by B.importance asc,A.Start')
write.table(tmp,file="COAD_DMR_20190413.txt",sep="\t",col.names=T,row.names=F,quote=F)
