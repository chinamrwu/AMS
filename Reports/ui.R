shinyUI(fluidPage(
   ...
   fluidRow(  
	   column(3,
		      fluidRow(column(7,selectInput("cancerCode", "癌症名称:", 
                choices = c(
						 '结直肠癌'   ='CRCA',
						 '胃 癌'      ='STAD',
						 '子宫内膜癌' ='UCEC',
				  ...
				  column(5,
                 downloadButton("downloadDMR","Download DMR")
				  )
				),
				selectInput("site", "选择一个病灶位置:",choices="overall"
				 ),

				sliderInput("adjCancer","癌旁Beta不超过:", min=0, max=1, value=0.5,step=0.1),
				sliderInput("dltB", "dltBeta不小于:", min = 0.1, max = 1, value = 0.1,step=0.1),
				fluidRow(
        ...
        tabPanel("DMR",dataTableOutput('DMR')),
		  tabPanel("Sample Information", dataTableOutput('sampleInf')),
    ...
    HTML("<footer style='width:100%;margin-left:auto;margin-right:auto;text-align:center'>
            <span>Copyright<span  style='font-family:Microsoft yahei'> &copy; </span>2019-2029.Wuhan Ammunition 
