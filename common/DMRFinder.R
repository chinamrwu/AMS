...
kk <- sapply(DMRScores$probeInf,function(v){a <- strsplit(v,'#')[[1]];c(a[3],a[2],a[1])})
...
write.csv(selection,file=sprintf("Squamour_ESAD_top%d_DMR_20190422.csv",dim(selection)[1]),row.names=F,quote=F)
