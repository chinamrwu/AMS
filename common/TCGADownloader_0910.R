...
rownames(probInfo) <- probInfo$probeId
...
island <- probInfo[!probInfo$Chromosome %in% c('*','chrX','chrY') & probInfo$Gene_Symbol !='.' & probInfo$Feature_Type=='Island',]
rnames <- island[,1]
X <- X[rnames,]
...
isLands <- probInfo[rnames,c('probeId','CGI_Coordinate')]

t0 <- sapply(unique(isLands[,2]),function(v){
   r1 <-  isLands$probeId[isLands$CGI_Coordinate==v]
   v0 <- apply(df0[r1,],2,mean)
	v0
})
