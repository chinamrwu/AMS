   ...
   probInf <- probInf[probInf$probeId %in% colnames(mat450),]
    ...
    #k1 <- apply(mat450[cancerIndex,],2,mean)
	 ...
	 crs <- sapply(isLands,function(v){
				...
				Regions <- c()
				index <- which(d1 >= 0.2)  ##################################  key cutoff to break continuous CpG sites into regions
				if(length(index)>0){
					index <- c(1,index+1)
					...
					for(i in 2:length(index)){
					  ...
					  if(w>=3){
						  ...
						  Width <- as.character(probInf[prbs[index[i-1]-1],'End']-probInf[prbs[index[i-1]],'Start'])
...
if(F){
  ...
  READ <- fread('F:/projects/TCGA/data/READ_450k.txt',header=T,sep="\t",stringsAsFactors=F,check.names=F)
   X <- cbind(as.data.frame(COAD),as.data.frame(READ[,-1]));
   DMR  <- findDMR(X,probInfPath='F:/projects/TCGA/data/TCGA_450K_sample.txt')

}
### tunning the codes with gene SDC2 and AMPH as reference 



if(F){
		rownames(X) <- X$probeId
		...
		X <- X[probInf$probeId,]
		R0  <- apply(X[,-1],1,function(v){sum(is.na(v))})
		X <- X[R0==0,-1]
		X <- data.frame(t(X))
